...
const AddNewsletterModal = ({ isOpen, onClose, onResourceAdded }) => {
  ...
  const handleUpload = async () => {
    if (!title || !coverImage || !pdfFile) {
      setError("Please provide a title, cover image, and PDF file.");
      return;
    }
    ...
    try {
      // Upload cover image
      const coverImagePath = `resources/newsletters/${title}/cover/${coverImage.name}`;
      const coverImageRef = ref(storage, coverImagePath);
      const coverImageUpload = uploadBytesResumable(coverImageRef, coverImage);
      ...
      coverImageUpload.on('state_changed',
        (snapshot) => {
      ...
      setError("Failed to add newsletter. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-gray-700/30 flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-md">
        <h3 className="text-lg font-semibold text-gray-800 mb-4">Add Newsletter</h3>
          ...
          <div>
            ...
            <div className="mt-1 border-2 border-dashed border-gray-300 rounded-md p-4 text-center">
              <input
                ...
                accept="image/*"
              />
              <label htmlFor="cover-image-upload" className="cursor-pointer text-green-700 hover:text-green-800 font-semibold flex items-center justify-center">
                <PlusIcon className="h-5 w-5 mr-2" /> Select Cover Image
              </label>
              {coverImage && (
                <div className="mt-2">
                  <p className="text-sm text-gray-600">{coverImage.name}</p>
                  {uploadProgress.coverImage !== undefined && (
                    <p className="text-sm text-gray-600">Uploading: {uploadProgress.coverImage.toFixed(0)}%</p>
                  )}
                </div>
              )}
            </div>
